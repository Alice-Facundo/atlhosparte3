...
fun RegisterScreen(mainNavController: NavHostController) {
    ...
    NavHost(
        ...
        // Rota para a tela final de resumo ou para a MainScreen do app
        composable("register_summary_or_main") {
            // Se esta for a última etapa e você quiser ir para a tela principal do app:
            mainNavController.navigate("main") {
                popUpTo("register") { inclusive = true } // Limpa o back stack do cadastro
            }
            // Ou, se tiver uma tela de resumo final:
            // RegisterSummaryScreen(registerNavController, viewModel, mainNavController)
        }
    }
}
